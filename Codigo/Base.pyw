...
def resetar_jogo():
    if messagebox.askyesno("Resetar", "Tem certeza que quer resetar todo o progresso?"):
        if os.path.exists(SAVE_FILE):
            os.remove(SAVE_FILE)
        janela.destroy()
...
reset_button = tk.Button(janela, text="🔁 Resetar Progresso", command=resetar_jogo, bg="#FFCCCC", fg="black", relief="raised", bd=3)
